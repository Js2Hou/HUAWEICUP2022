...
from processes import assemble_orders, item2stack, stack2stride_ga, stride2ban_ga
...
for dataset_idx in range(1, 6):
    output_dir = os.sep.join(
        ['outputs', f'B{dataset_idx}', f'v3_window({N1}x{N2})_ga'])
    ...
    # 建立订单信息表字典, key为订单名称, v为订单内items总长度和面积
    orders_detail = {}
    for e in data:
        order_name_ = e[-1]
        _s = float(e[3]) * float(e[4])
        if order_name_ in orders_detail.keys():
            orders_detail[order_name_][0] += 1
            orders_detail[order_name_][1] += _s
        else:
            orders_detail[order_name_] = np.array([1, _s])
    ...
    for batch_id, batch_orders_name in enumerate(orders_placement):
        ...
        for material_name_ in batch_material_set_:
            ...
            # 栈 >> 条带
            strides, stack_dt = stack2stride_ga(stacks, constH, constW, N1)
            # 条带>> 板
            bans, stride_dt = stride2ban_ga(strides, constH, constW, N2)

            n_bans += bans.shape[0]

            # 暂存便于后面作图统计
            cur_batch_.append(
                [material_name_, bans, item_dt, stack_dt, stride_dt])
    ...
    # 结果保存值csv文件
    title = ['batch_id', 'board_material', 'board_id',
             'item_id', 'x', 'y', 'item_length', 'item_width']
    res4csv = [title]
    id_rawp = -1  # 全局原片id
        ...
        for id_material, material_data_ in enumerate(batch_):
            ...
            for i in range(bans.shape[0]):
                ...
                for k, v in cordt.items():
                    ...
                    tem_ = [id_batch, board_material, id_rawp,
                            k, left, bottom, height, width]
                    res4csv.append(tem_)
