...
def assemble_orders(orders, n_orders, wsize):
    # 将订单组批
    orders_placement = []  # [['order1', 'order2'], ['order6', 'order4']]
    sorted_orders = orders.copy()
    np.random.shuffle(sorted_orders)
    rest_idx_orders = list(range(n_orders))  # unselected idx of stacks

    while rest_idx_orders:
        # idx_stacks_window_同rest_idx_stacks，为后者的子集
        idx_orders_window_ = rest_idx_orders[:wsize]
        x_ = sorted_orders[idx_orders_window_, 1:].astype(np.float32)
        
        # res: (1, N1), 仅包括0或者1, 1表示被用掉
        res = order_assemble_solver(x_).flatten()
        ...
        for widx_ in res:
            ...
            cur_order_ = sorted_orders[idx_orders_window_[widx_]]
            p.append(str(cur_order_[0])) # 添加订单名称
        
        for widx_ in res[::-1]:
            rest_idx_orders.pop(widx_)
        orders_placement.append(p)
    return orders_placement
...
def stack2stride_ga(stacks, constH, constW, wsize):
    ...
    while rest_idx_stacks:
        ...
        for widx_ in res:
            ...
            # if cur_stack_[0] in set_stack_selected:
            #     print(f'warning: stack {cur_stack_[0]} appears twice.')
            #     continue

            # set_stack_selected.add(cur_stack_[0])
